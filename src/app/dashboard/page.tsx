...
export default function DashboardPage() {
  ...
  async function loadAll(uid: string) {
    ...
    const savedRes = await supabase
      .from('saved_projects')
      .select('*, projects:project_id(*, profiles:founder_id(display_name, avatar_url))')
      .eq('profile_id', uid)
      .order('created_at', { ascending: false })
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0F172A', padding: '2rem 1rem' }}>
      <div style={{ maxWidth: 860, margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', gap: 4, borderBottom: '1px solid #2D3F5C', marginBottom: 28, overflowX: 'auto' as const }}>
          {([['ricevute', 'Ricevute'], ['inviate', 'Inviate'], ['mie-idee', 'Le mie idee'], ['preferiti', '⭐ Preferiti']] as [Tab, string][]).map(([t, label]) => (
            <button key={t} onClick={() => setTab(t)} style={{
              ...
              fontWeight: tab === t ? 500 : 400, marginBottom: -1, whiteSpace: 'nowrap' as const,
            }}>{label}</button>
          ))}
        </div>
        ...
        {!loading && tab === 'preferiti' && (
          <div>
            {savedProjects.length === 0
              ? <Empty text="Nessuna idea salvata nei preferiti" />
              : savedProjects.map(s => {
                const pr = s.projects
                if (!pr) return null
                const isComplete = pr.status === 'closed'
                return (
                  <div key={s.id} style={{ background: '#1E293B', border: `1px solid ${isComplete ? 'rgba(16,185,129,0.4)' : '#2D3F5C'}`, borderRadius: 14, padding: '1.25rem', marginBottom: 14 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                      <div>
                        <div style={{ fontSize: 16, fontWeight: 600, color: '#F1F5F9', marginBottom: 4 }}>{pr.title}</div>
                        <div style={{ fontSize: 12, color: '#64748B' }}>{pr.category}</div>
                      </div>
                      {isComplete
                        ...
                        : <span style={{ fontSize: 11, padding: '3px 10px', borderRadius: 999, background: 'rgba(245,158,11,0.12)', border: '1px solid rgba(245,158,11,0.3)', color: '#F59E0B' }}>Aperta</span>
                      }
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
                      <Avatar url={pr.profiles?.avatar_url} name={pr.profiles?.display_name} size={28} />
                      <span style={{ fontSize: 13, color: '#94A3B8' }}>{pr.profiles?.display_name ?? 'Founder'}</span>
                    </div>
                    <div style={{ display: 'flex', gap: 8 }}>
                      {!isComplete && (
                        <a href={`/projects/${pr.id}`} style={{ ...btn, display: 'inline-block', textDecoration: 'none', fontSize: 13 }}>
                          Partecipa →
                        </a>
                      )}
                      <button onClick={async () => {
                        await supabase.from('saved_projects').delete().eq('id', s.id)
                        setSavedProjects(p => p.filter(x => x.id !== s.id))
