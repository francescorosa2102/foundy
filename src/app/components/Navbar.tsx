    ...
    <>
      <nav style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          >
            <span style={{ display: 'block', width: 22, height: 2, background: '#F1F5F9', borderRadius: 2, transition: 'all 0.2s', transform: mobileOpen ? 'rotate(45deg) translateY(7px)' : 'none' }} />
            <span style={{ display: 'block', width: 22, height: 2, background: '#F1F5F9', borderRadius: 2, opacity: mobileOpen ? 0 : 1, transition: 'all 0.2s' }} />
            <span style={{ display: 'block', width: 22, height: 2, background: '#F1F5F9', borderRadius: 2, transition: 'all 0.2s', transform: mobileOpen ? 'rotate(-45deg) translateY(-7px)' : 'none' }} />
          </button>
        </div>
      </nav>
      ...
      {mobileOpen && (
        <div style={{
          ...
          padding: '2rem 1.5rem', gap: 8,
        }} className="mobile-nav">
          {links.map(l => (
            <Link key={l.href} href={l.href} onClick={() => setMobileOpen(false)} style={{
              padding: '14px 16px', borderRadius: 10, fontSize: 18,
          ...
          {user ? (
            <>
              <Link href="/profile" onClick={() => setMobileOpen(false)} style={{ padding: '14px 16px', borderRadius: 10, fontSize: 18, color: '#F1F5F9' }}>Il mio profilo</Link>
              <button onClick={async () => { await supabase.auth.signOut(); setMobileOpen(false); window.location.href = '/' }}
                style={{ padding: '14px 16px', borderRadius: 10, fontSize: 18, color: '#EF4444', background: 'none', border: 'none', cursor: 'pointer', textAlign: 'left' }}>
                Esci
              </button>
            </>
          ) : (
            <>
              <Link href="/login" onClick={() => setMobileOpen(false)} style={{ padding: '14px 16px', borderRadius: 10, fontSize: 18, color: '#F1F5F9', background: '#1E293B' }}>Accedi</Link>
              <Link href="/signup" onClick={() => setMobileOpen(false)} style={{ padding: '14px 16px', borderRadius: 10, fontSize: 18, color: '#fff', background: '#7C3AED', fontWeight: 500, textAlign: 'center' }}>Registrati</Link>
            </>
          )}
        </div>
      )}
