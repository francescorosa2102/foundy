...
export default function Home() {
  ...
  const [np, setNp] = useState({ title: '', description: '', category: 'Tech', required_roles: '', image_url: '' })
  ...
  async function createProject() {
    ...
    const { error } = await supabase.from('projects').insert({
      ...
      category: np.category, required_roles: roles, status: 'active',
      image_url: np.image_url || null,
    })
    if (error) { showToast('Errore: ' + error.message); return }
    showToast('Progetto creato! 🎉')
    setShowNew(false)
    setNp({ title: '', description: '', category: 'Tech', required_roles: '', image_url: '' })
    loadProjects()
  }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 24 }}>
      ...
      <div style={{ display: 'flex', gap: 12 }}>
        ...
        {!loading && projects.map(pr => (
          <div key={pr.id} style={{ background: '#1E293B', border: '1px solid #2D3F5C', borderRadius: 16, overflow: 'hidden', marginBottom: 20 }}>
            {pr.image_url
            ? <img src={pr.image_url} alt={pr.title} style={{ width: '100%', height: 200, objectFit: 'cover' }} />
            : <div style={{ height: 160, background: getCategoryGradient(pr.category), display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
      <img src="/foundy.png" alt="Foundy" style={{ height: 100, width: 'auto', opacity: 0.7 }} />
    </div>
}
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                ...
                {user?.id === pr.founder_id
                  ...
                  : sentIds.includes(pr.id)
                    ...
                    : <button onClick={() => { if (!user) { window.location.href = '/login'; return } setJoinModal(pr) }} style={btn}>Partecipa →</button>
                }
              </div>
            </div>
          </div>
